...
class MotivationNN(nn.Module):
    def __init__(self):
        ...
        # Trainable "mind" with a hidden layer for better convergence
        self.mind_hidden = nn.Linear(1, 10)  # Input to hidden layer (more capacity)
        self.mind_output = nn.Linear(10, 3)  # Hidden to 3 control neurons

        # Fixed "hands" layer: maps vectors to distinguishable outputs
        self.hands = nn.Linear(3, 1, bias=False)
        with torch.no_grad():
            self.hands.weight = nn.Parameter(torch.tensor([[1.5, 3.0, 4.5]]), requires_grad=False)

    def forward(self, x):
        # x is the desired category (e.g., 5)
        hidden = torch.relu(self.mind_hidden(x))  # Hidden layer with ReLU
...
def compute_loss(control, target_category):
    ...
    return torch.mean((control - target_vector) ** 2)  # MSE to target vector


# Training loop
def train_model(num_epochs=5000):  # More epochs for convergence
    for epoch in range(num_epochs):
        ...
        # Every 500 epochs, print progress
        if (epoch + 1) % 500 == 0:
