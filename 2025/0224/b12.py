...
class MotivationNN(nn.Module):
    def __init__(self):
        ...
        self.mind_hidden = nn.Linear(1, 16)  # Increased hidden neurons for capacity
...
model = MotivationNN()
optimizer = optim.Adam(model.parameters(), lr=0.05)  # Higher initial lr
scheduler = lr_scheduler.StepLR(optimizer, step_size=1000, gamma=0.5)  # Reduce lr every 1000 epochs


# Loss function: stronger penalty for deviations
def compute_loss(control, target_category):
    target_vector = PREDEFINED_VECTORS[target_category.long()]
    # Use a higher power to penalize small errors more
    return torch.mean((control - target_vector) ** 4)  # Quartic loss for sharper convergence


# Training loop
def train_model(num_epochs=10000):  # More epochs for fine-tuning
