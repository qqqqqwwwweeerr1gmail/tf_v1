...
class MotivationNN(nn.Module):
    def __init__(self):
        ...
        # Fixed "hands" layer: takes 3 neurons (sum 0-3) and produces output in [0, 9]
        self.hands = nn.Linear(3, 1, bias=False)  # No bias for simplicity
        with torch.no_grad():
            self.hands.weight = nn.Parameter(torch.tensor([[3.0, 3.0, 3.0]]), requires_grad=False)  # Sum scales to 9

    def forward(self, x):
        # x is the desired output (e.g., 5)
        control = torch.sigmoid(self.mind(x))  # 3 control neurons, each in [0, 1]
        output = self.hands(control)  # Fixed hands operation, maps sum [0, 3] to [0, 9]
