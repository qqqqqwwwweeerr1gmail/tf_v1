...
# game_df['steps_moving_average'] = game_df['steps'].rolling(window=1).mean()
# ax = game_df.plot('game','steps_moving_average', figsize=(10,10), legend=False)
ax = game_df.plot('game','steps_moving_average', figsize=(5,5), legend=False)
ax.set_xlabel('Game')
ax.set_ylabel('Steps')
plt.show()

game_df['actor_cost_moving_average'] = game_df['actor_cost'].rolling(window=50).mean()
# game_df['actor_cost_moving_average'] = game_df['actor_cost'].rolling(window=1).mean()
...
# game_df['critic_cost_moving_average'] = game_df['critic_cost'].rolling(window=1).mean()
...
while not game_over and steps< 3000:
    steps += 1
    state = np.copy(next_state)
...
import pickle
with open('./video/b4_i1.pkl', 'wb') as file:
    pickle.dump(images, file)

import pickle
with open('./video/b4_a1.pkl', 'wb') as file:
