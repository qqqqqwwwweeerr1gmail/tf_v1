...
class MotivationNN(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.hands(control)  # Reference output
...
def train_model(num_epochs=10000):
    for epoch in range(num_epochs):
        # Randomly pick an operation (0-9)
        idx = torch.randint(0, 10, (1,)).item()
        action_will = ACTION_WILL_VECTORS[idx:idx + 1]  # 1x4
        desired = torch.tensor([idx], dtype=torch.long)

        output, control = model(action_will)
