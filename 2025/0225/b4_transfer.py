...
HAND_COMMAND_VECTORS = torch.tensor([
    ...
    [1.0, 1.0, 1.0],  # Operation 9
], dtype=torch.float32)


# Define the Neural Network
class MotivationNN(nn.Module):
    def __init__(self, model_path="motivation_model.pth"):
        ...
        # Xavier initialization only if no saved model exists
...
def train_model(model, num_epochs=10000, force_train=False):
    # Check if model exists and training isn't forced
...
train_model(model, num_epochs=10000, force_train=False)  # Set force_train=True to retrain
evaluate_model(model)
model.save_model()
