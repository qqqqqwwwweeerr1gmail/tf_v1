...
class MotivationNN(nn.Module):
    def __init__(self):
        ...
        # Fixed "hands" layer: takes 3 neurons (each 0-1) and produces output in [0, 9]
        self.hands = nn.Linear(3, 1, bias=False)  # No bias for simplicity
        with torch.no_grad():
            self.hands.weight = nn.Parameter(torch.tensor([[1.5, 3.0, 4.5]]), requires_grad=False)  # Unique weights

    def forward(self, x):
        # x is the desired output (e.g., 5)
        control = torch.sigmoid(self.mind(x))  # 3 control neurons, each in [0, 1]
        output = self.hands(control)  # Fixed hands operation, maps to [0, 9]
        return output, control  # Return raw output and control neurons
...
# Training loop
def train_model(num_epochs=2000):  # Increased epochs for better convergence
    for epoch in range(num_epochs):
        ...
        # Every 200 epochs, print progress including brain output
        if (epoch + 1) % 200 == 0:
...
test_model()
train_model(num_epochs=2000)  # More epochs to ensure convergence
test_model()
train_model(num_epochs=20000)  # More epochs to ensure convergence
test_model()
