...
class DDQNAgent:
    ...
    def learn(self, experiences):
        ...
        # a = self.qnetwork_target(next_states)
        # b = self.qnetwork_target(next_states).detach()
        # c = self.qnetwork_target(next_states).detach().max(1)
        # d = self.qnetwork_target(next_states).detach().max(1)[0]
        # e = self.qnetwork_target(next_states).detach().max(1)[0].unsqueeze(1)
        ...
        for name, param in self.qnetwork_target.named_parameters():
            if param.requires_grad:
                print(name, param.data)
